	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( 'mega' === $menu_type || 'regular' === $menu_type ) {
			...
			if ( ! empty( $item_image ) ) {
				...
				$link_classes [] = $container_class . ' ' . ( isset( $image_placeholderr['o'] ) ? "tcb--{$image_placeholderr['o']}" : 'tcb--row' );
				$item_image      = array(
					'placeholder' => true,
					'o'           => isset( $image_placeholderr['o'] ) ? $image_placeholderr['o'] : '',
				);
			} elseif ( ! empty( $image_placeholder[ $place ] ) && isset( $image_placeholder[ $place ]['enabled'] ) && $image_placeholder[ $place ]['enabled'] ) {
				$link_classes [] = $container_class . ' ' . ( isset( $image_placeholderr[ $place ]['o'] ) ? "tcb--{$image_placeholderr[ $place ]['o']}" : 'tcb--row' );
				$item_image      = array(
					'placeholder' => true,
					'o'           => isset( $image_placeholderr[ $place ]['o'] ) ? $image_placeholderr[ $place ]['o'] : '',
				);
			}
		}
