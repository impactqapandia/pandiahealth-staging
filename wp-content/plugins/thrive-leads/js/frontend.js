		...
		if ( response.html ) {
			...
			ThriveGlobal.$j.each( response.html, function ( elem_type, html ) {
				...
				if ( elem_type === 'in_content' ) {
					...
					var p = post.find( 'p' ).filter( ':visible' ).filter( ':not(.thrv_table *, form *, .tcb-post-list *)' );

					if ( p.length === 0 && response.in_content_pos == 0 ) {
						ThriveGlobal.$j( '.tve-tl-cnt-wrap' ).prepend( html );
					} else {
						if ( parseInt( response.in_content_pos ) === 0 ) {
							response.in_content_pos = 1;
							fn = 'before';
						}

						var $target = p.eq( parseInt( response.in_content_pos ) - 1 );
						var $textEl = $target.closest( '.thrv_text_element' );
