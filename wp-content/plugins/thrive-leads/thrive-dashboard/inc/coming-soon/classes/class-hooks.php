...
class Hooks {
	...
	public static function actions() {
		...
		add_action( 'init', array( __CLASS__, 'disable_coming_soon' ) );
	}
    ...
    */
	...
	public static function admin_enqueue_scripts( $screen ) {
		if ( ! empty( $screen ) && $screen === 'admin_page_tve_dash_coming_soon' ) {
			$coming_soon_page_id = get_option( 'tvd_coming_soon_page_id', 0 );
			tve_dash_enqueue();
	...
	public static function disable_coming_soon() {
		...
		if ( ! isset( $enabled['coming-soon'] ) || get_post_status( get_option( 'tvd_coming_soon_page_id', 0 ) ) !== 'publish' ) {
			update_option( 'tvd_coming_soon_page_id', 0 );
		}
	}
	...
	/**
	 * Do not allow TL forms to be shown on the selected Coming Soon Page
	 *
	 * @param array  $items
